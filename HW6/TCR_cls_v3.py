...
FILE_AUCs = DIRIO + opt.data_set + '_AUC_v3.txt'
FILE_model = DIRIO + opt.data_set + '_model_v3.pth'
...
def seq_process(seq, randn = False):
    ...
    for i in range(len_seq):
        x[amino_acid[seq[i]]][i+pad_len] = 1 + (np.random.standard_normal()/20 if randn else 0)
    return x
